"""
generate_face_walk_frames.py - Generador de frames de caminata para Ojos, Nariz, Boca, Cabeza y Pelo
...
def generate_walk_frames_for_category(category: str):
    ...
    # Buscar archivos estáticos (ej: cateyes1.png a cateyes8.png)
    files = [f for f in os.listdir(cat_dir) if f.endswith(".png") and "_walk" not in f and "_f" not in f]
    count_generated = 0
    ...
    for fname in files:
        ...
        for f_idx, (dx, dy) in enumerate(offsets):
            ...
            if os.path.exists(ase_cat_dir):
                ase_dest = os.path.join(ase_cat_dir, dest_name)
                shifted.save(ase_dest)

    print(f"[OK] Creados {count_generated} frames de caminata para '{category}'.")
...
def generate_walk_frames_for_hair():
    ...
    count_generated = 0
    for style_name in os.listdir(hair_dir):
        style_dir = os.path.join(hair_dir, style_name)
        ...
        for sub in ("front", "back"):
            ...
            for fname in os.listdir(sub_dir):
                if "_walk" in fname or "_f" in fname or not fname.endswith(".png"):
                    continue
                ...
                for f_idx, (dx, dy) in enumerate(offsets):
                    ...
                    # Sincronizar también a Downloads si existe
                    ase_sub_dir = os.path.join(ASEPRITE_DOWNLOADS_DIR, "hair", style_name, sub)
                    if os.path.exists(ase_sub_dir):
                        ase_dest = os.path.join(ase_sub_dir, dest_name)
                        shifted.save(ase_dest)

    print(f"[OK] Creados {count_generated} frames de caminata para 'hair' (front y back).")

def regenerate_all_walk_frames():
    print("=== Regenerando frames de caminata para Ojos, Nariz, Boca, Cabeza y Pelo ===")
